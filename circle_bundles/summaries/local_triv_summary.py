# circle_bundles/summaries/local_triv_summary.py
...
from ..trivializations.local_triv import LocalTrivResult

if TYPE_CHECKING:
    from ..analysis.quality import BundleQualityReport


# --- helpers copied from characteristic_class.py style ---
...
@dataclass
class LocalTrivSummary:
    """
    A lightweight summary: (Diagnostics).

    The Diagnostics portion is formatted to match the *old characteristic class summary*
    Diagnostics block (same labels/ordering), except we do NOT include Euler rounding.

    NOTE: We intentionally omit the "Local Trivializations" bookkeeping section
    (valid patches / retries / landmarks / errors) to keep output focused.
    """
    n_sets: int
    n_samples: int
...
def _display_summary_latex(summary: LocalTrivSummary) -> bool:
    """
    Best-effort IPython Math display.

    Matches the characteristic_class summary *Diagnostics* block formatting (minus Euler rounding).
    NOTE: We intentionally omit the Local Trivializations bookkeeping block.
    """
    try:
    ...
    # Diagnostics rows (same style as characteristic_class)
